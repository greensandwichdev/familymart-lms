...
def get_members(start=0, search=""):
	...
	rank_map = {r.name: r.rank_name for r in frappe.get_all("Store Rank", fields=["name", "rank_name"])}

	store_map = {s.name: s.store_name for s in frappe.get_all("LMS Store", fields=["name", "store_name"])}
...
@frappe.whitelist()
def get_store_list():
	"""Returns the list of active stores for dropdown."""
...
@frappe.whitelist()
def get_rank_list():
	"""Returns the list of store ranks for dropdown."""
